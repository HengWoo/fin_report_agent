...
from ..models.financial_data import IncomeStatement, FinancialPeriod, RevenueBreakdown, CostBreakdown, ExpenseBreakdown, ProfitMetrics
...
class FinancialAnalysisTools:
    ...
    async def parse_excel_file(
        ...
        if file_size_mb > self.config.max_file_size_mb:
            raise ValueError(f"File size ({file_size_mb:.1f} MB) exceeds limit ({self.config.max_file_size_mb} MB)")

        # Check file extension
        if path_obj.suffix.lower() not in self.config.allowed_file_extensions:
            raise ValueError(f"File extension {path_obj.suffix} not allowed. Allowed: {self.config.allowed_file_extensions}")
        ...
        try:
            ...
            for period_id, period_data in financial_data.items():
                try:
                    ...
                    income_statement = self._build_income_statement_from_data(period_id, period_data)

                    # Validate the income statement
                    period_validation = self.validator.validate_income_statement(income_statement)
    ...
    async def calculate_restaurant_kpis(
        ...
        try:
            ...
            kpi_dict = {
                "profitability": kpis.profitability.__dict__ if hasattr(kpis.profitability, '__dict__') else kpis.profitability,
                "efficiency": kpis.efficiency.__dict__ if hasattr(kpis.efficiency, '__dict__') else kpis.efficiency,
                "growth": kpis.growth.__dict__ if hasattr(kpis.growth, '__dict__') else kpis.growth,
                "liquidity": kpis.liquidity.__dict__ if hasattr(kpis.liquidity, '__dict__') else kpis.liquidity,
                "risk": kpis.risk.__dict__ if hasattr(kpis.risk, '__dict__') else kpis.risk
            }
        ...
        try:
            ...
            return {
                ...
                "trends": {
                    ...
                    "trend_direction": trend_result.trend_summary.get("overall_direction", "stable"),
    ...
    async def generate_business_insights(
        ...
        try:
            ...
            insights = insights_generator.generate_insights_from_kpis(kpi_obj, income_statement)
            ...
            return {
                "success": True,
                "language": language,
                "bilingual_output": self.config.enable_bilingual_output and language == "both",
    ...
    async def comprehensive_analysis(
        ...
        try:
            ...
            if language != "both":
                analysis_data = self._format_analysis_by_language(analysis_data, language)
    ...
    def _build_income_statement_from_data(self, period_id: str, period_data: Dict[str, Any]) -> IncomeStatement:
    ...
    def _format_analysis_by_language(self, analysis_data: Dict[str, Any], language: str) -> Dict[str, Any]:
